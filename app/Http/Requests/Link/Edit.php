...
class Edit extends FormRequest
{
    ...
    public function rules()
    {
        ...
        $id = $this->route('id');
        $link = Link::with('descendants')->findOrFail($id);
        ...
        if ($is_locale) {
            $rules['code'][] = Rule::unique('links', 'code')->where(function ($query) use ($locale, $id) {
                $query->where('locale', '=', $locale)->where('id', '!=', $id);
            });
            if ($is_lg)
                $rules['parent_id'][] = Rule::exists('links', 'id')->where(function ($query) use ($locale, $group, $disallowed) {
                    $query->where('locale', '=', $locale)->where('group', '=', $group)->whereNotIn('id', $disallowed);
                });
        }
